...
class Indicator_PriceChannels:
    """Price Channels - Highest high / Lowest low channels"""
    PARAMETERS = {
    ...
    def __init__(self):
        self.name, self.category, self.version = "PriceChannels", "Trend", __version__
    ...
    def generate_signals_dynamic(self, data, params):
        ...
        return {'entries': entries, 'exits': exits, 'exit_reason': pd.Series('channel_cross', index=data.index),
                'signal_strength': (data['close'] - channels['lower']) / (channels['upper'] - channels['lower'] + 1e-10)}
    
    def get_ml_features(self, data, params):
        channels = self.calculate(data, params)
        return pd.DataFrame({
            'pc_upper': channels['upper'],
            'pc_middle': channels['middle'],
            'pc_lower': channels['lower'],
            'pc_position': (data['close'] - channels['lower']) / (channels['upper'] - channels['lower'] + 1e-10),
            'pc_width': channels['upper'] - channels['lower']
        }, index=data.index)
