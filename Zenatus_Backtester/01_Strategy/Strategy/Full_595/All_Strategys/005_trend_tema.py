"""
005 - Triple Exponential Moving Average (TEMA)
Noch geringerer Lag durch dreifache EMA-Berechnung
"""
...
class Indicator_TEMA:
    """
    Triple Exponential Moving Average (TEMA) - Trend
    
    Beschreibung:
        TEMA = 3*EMA - 3*EMA(EMA) + EMA(EMA(EMA))
        Minimiert Lag noch stärker als DEMA.
    ...
    def calculate(self, data: pd.DataFrame, params: Dict) -> pd.Series:
        """
        Berechnet Triple Exponential Moving Average.
    ...
    def generate_signals_fixed(self, data: pd.DataFrame, params: Dict) -> Dict[str, pd.Series]:
        ...
        tema = self.calculate(data, params)
        entries = (data['close'] > tema) & (data['close'].shift(1) <= tema.shift(1))
        ...
        signal_strength = abs(data['close'] - tema) / tema
    ...
    def generate_signals_dynamic(self, data: pd.DataFrame, params: Dict) -> Dict[str, pd.Series]:
        ...
        tema = self.calculate(data, params)
        entries = (data['close'] > tema) & (data['close'].shift(1) <= tema.shift(1))
        exits = (data['close'] < tema) & (data['close'].shift(1) >= tema.shift(1))
        
        exit_reason = pd.Series('', index=data.index)
        exit_reason[exits] = 'tema_reverse_cross'
    ...
    def get_ml_features(self, data: pd.DataFrame, params: Dict) -> pd.DataFrame:
        ...
        features['tema_value'] = tema
        rolling_min = tema.rolling(100, min_periods=1).min()
        rolling_max = tema.rolling(100, min_periods=1).max()
        features['tema_normalized'] = (tema - rolling_min) / (rolling_max - rolling_min + 1e-10)
        features['tema_slope'] = tema.diff()
        features['tema_acceleration'] = tema.diff().diff()
        features['distance_from_price'] = data['close'] - tema
        features['distance_pct'] = (data['close'] - tema) / tema * 100
        features['above_tema'] = (data['close'] > tema).astype(int)
