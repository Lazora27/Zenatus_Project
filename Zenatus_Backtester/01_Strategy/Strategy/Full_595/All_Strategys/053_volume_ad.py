"""053 - Accumulation/Distribution (A/D)"""
import numpy as np
import pandas as pd
...
class Indicator_AD:
    ...
    PARAMETERS = {
        'signal_period': {'default': 20, 'min': 5, 'max': 50, 'values': [5,7,8,11,13,14,17,19,20,21,23,29,31,34,37,41,43,47], 'optimize': True, 'ml_feature': True},
        'tp_pips': {'default': 50, 'values': [30,40,50,60,75,100,125,150,200], 'optimize': True},
        'sl_pips': {'default': 25, 'values': [10,15,20,25,30,40,50], 'optimize': True}
    }
    def __init__(self):
        self.name, self.category, self.version = "AD", "Volume", __version__
    def calculate(self, data: pd.DataFrame, params: Dict) -> pd.DataFrame:
        ...
        ad_signal = ad.ewm(span=signal_period, adjust=False).mean()
        return pd.DataFrame({'ad': ad, 'ad_signal': ad_signal, 'ad_divergence': ad - ad_signal}, index=data.index)
    def generate_signals_fixed(self, data: pd.DataFrame, params: Dict) -> Dict[str, pd.Series]:
        result = self.calculate(data, params)
        ad, ad_signal = result['ad'], result['ad_signal']
        entries = (ad > ad_signal) & (ad.shift(1) <= ad_signal.shift(1))
        ...
        return {'entries': entries, 'exits': exits, 'tp_levels': pd.Series(np.nan, index=data.index), 
                'sl_levels': pd.Series(np.nan, index=data.index), 'signal_strength': abs(result['ad_divergence']).clip(0, 1000000) / 1000000}
    def generate_signals_dynamic(self, data: pd.DataFrame, params: Dict) -> Dict[str, pd.Series]:
        result = self.calculate(data, params)
        ad, ad_signal = result['ad'], result['ad_signal']
        entries, exits = (ad > ad_signal) & (ad.shift(1) <= ad_signal.shift(1)), (ad < ad_signal) & (ad.shift(1) >= ad_signal.shift(1))
        return {'entries': entries, 'exits': exits, 'exit_reason': pd.Series('ad_signal_cross', index=data.index),
                'signal_strength': abs(result['ad_divergence']).clip(0, 1000000) / 1000000}
    def get_ml_features(self, data: pd.DataFrame, params: Dict) -> pd.DataFrame:
        result = self.calculate(data, params)
        return pd.DataFrame({'ad_value': result['ad'], 'ad_signal': result['ad_signal'], 'ad_divergence': result['ad_divergence'],
                            'ad_slope': result['ad'].diff(), 'ad_trend': (result['ad'] > result['ad_signal']).astype(int)}, index=data.index)
