"""
002 - Exponential Moving Average (EMA)
...
class Indicator_EMA:
    ...
    Beschreibung:
        Gewichteter Durchschnitt mit exponentiell abnehmenden Gewichten.
    ...
    def generate_signals_fixed(self, data: pd.DataFrame, params: Dict) -> Dict[str, pd.Series]:
        ...
        ema = self.calculate(data, params)
        entries = (data['close'] > ema) & (data['close'].shift(1) <= ema.shift(1))
        ...
        signal_strength = abs(data['close'] - ema) / ema
    ...
    def generate_signals_dynamic(self, data: pd.DataFrame, params: Dict) -> Dict[str, pd.Series]:
        ...
        ema = self.calculate(data, params)
        entries = (data['close'] > ema) & (data['close'].shift(1) <= ema.shift(1))
        exits = (data['close'] < ema) & (data['close'].shift(1) >= ema.shift(1))
        
        exit_reason = pd.Series('', index=data.index)
        exit_reason[exits] = 'ema_reverse_cross'
    ...
    def get_ml_features(self, data: pd.DataFrame, params: Dict) -> pd.DataFrame:
        ...
        features['ema_value'] = ema
        rolling_min = ema.rolling(100, min_periods=1).min()
        rolling_max = ema.rolling(100, min_periods=1).max()
        features['ema_normalized'] = (ema - rolling_min) / (rolling_max - rolling_min + 1e-10)
        features['ema_slope'] = ema.diff()
        features['ema_acceleration'] = ema.diff().diff()
        features['ema_percentile'] = ema.rolling(100, min_periods=1).apply(
            lambda x: pd.Series(x).rank(pct=True).iloc[-1] if len(x) > 0 else 0.5
        )
        features['distance_from_price'] = data['close'] - ema
        features['distance_pct'] = (data['close'] - ema) / ema * 100
        features['above_ema'] = (data['close'] > ema).astype(int)
        features['cross_up'] = ((data['close'] > ema) & (data['close'].shift(1) <= ema.shift(1))).astype(int)
        features['cross_down'] = ((data['close'] < ema) & (data['close'].shift(1) >= ema.shift(1))).astype(int)
        
        atr = data['high'].rolling(14, min_periods=1).max() - data['low'].rolling(14, min_periods=1).min()
        features['volatility_adjusted'] = (data['close'] - ema) / (atr + 1e-10)
        ...
        for param_name, param_info in self.PARAMETERS.items():
            if param_name in params:
                ...
                if param_name == 'period' and not isinstance(value, (int, np.integer)):
                    raise ValueError(f"{param_name} must be integer")
