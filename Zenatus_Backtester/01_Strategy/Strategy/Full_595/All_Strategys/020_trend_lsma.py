"""
020 - Least Squares Moving Average (LSMA)
Identisch mit Linear Regression MA (Alias)
"""
...
class Indicator_LSMA:
    """Least Squares Moving Average (LSMA) - Trend"""
    ...
    def calculate(self, data: pd.DataFrame, params: Dict) -> pd.Series:
        """Berechnet LSMA (identisch mit LINREG)."""
        ...
        lsma = data['close'].rolling(window=period).apply(lsma_calc, raw=True)
        return lsma
    
    def generate_signals_fixed(self, data: pd.DataFrame, params: Dict) -> Dict[str, pd.Series]:
        lsma = self.calculate(data, params)
        entries = (data['close'] > lsma) & (data['close'].shift(1) <= lsma.shift(1))
        ...
        return {'entries': entries, 'exits': exits,
                ...
                'signal_strength': abs(data['close'] - lsma) / lsma}
    
    def generate_signals_dynamic(self, data: pd.DataFrame, params: Dict) -> Dict[str, pd.Series]:
        lsma = self.calculate(data, params)
        entries = (data['close'] > lsma) & (data['close'].shift(1) <= lsma.shift(1))
        exits = (data['close'] < lsma) & (data['close'].shift(1) >= lsma.shift(1))
        exit_reason = pd.Series('', index=data.index)
        exit_reason[exits] = 'lsma_reverse_cross'
    ...
    def get_ml_features(self, data: pd.DataFrame, params: Dict) -> pd.DataFrame:
        ...
        features['lsma_value'] = lsma
        features['lsma_slope'] = lsma.diff()
        features['distance_from_price'] = data['close'] - lsma
        return features
