"""
DEEP-ANALYSE: Indikatoren 21-40
...
JSON_FILE = Path("INDICATORS_1_20_ANALYSIS.json")
OUTPUT_JSON = Path("INDICATORS_COMPLETE_ANALYSIS.json")

def analyze_indicator_deep(ind_file: Path, target_combos: int) -> dict:
    ...
    result = {
        ...
        'methods': [],
        'optimal_inputs': {},
        'target_combinations': target_combos,
    ...
    # 5. Fallback: Suche nach Parameter-Verwendung
    ...
    # 6. Berechne optimale Inputs basierend auf Parameter-Anzahl
    num_params = len(result['parameters'])
    
    if num_params == 0:
        # Default: 1 Parameter (period)
        ...
        # Ziel: 500 Kombinationen → 8×8 × 4×4 = 1024
        ...
        # Ziel: 1000 Kombinationen → 6×6×6 × 3×3 = 1944
        # Reduziere auf: 5×5×5 × 4×4 = 2000
        param_names = list(result['parameters'].keys())
        ...
        entry_combos = 5 * 5 * 5  # 125
        result['calculated_combinations'] = entry_combos * 4 * 4  # 2000
        
    elif num_params == 4:
        # Ziel: 1000 Kombinationen → 4×4×4×4 × 3×3 = 2304
        param_names = list(result['parameters'].keys())
        ...
        entry_combos = 4 ** 4  # 256
        result['calculated_combinations'] = entry_combos * 3 * 3  # 2304
        
    else:
        # 5+ Parameter: Ziel 1500-2500
        # 3×3×3×3×3 × 4×4 = 3888
        values_per_param = 3
        ...
        entry_combos = values_per_param ** num_params
...
# MAIN
print("="*80)
print("DEEP-ANALYSE: INDIKATOREN 21-40")
print("="*80)
...
for i in range(21, 41):
    ...
    # Bestimme Ziel basierend auf erwarteter Parameter-Anzahl
    target = 500  # Default
    
    result = analyze_indicator_deep(ind_file, target)
    
    # Adjustiere Ziel basierend auf tatsächlicher Parameter-Anzahl
    num_params = len(result['parameters'])
    if num_params <= 2:
        ...
        result['target_combinations'] = 2000  # Mitte von 1500-2500
...
print(f"\n{'='*80}")
print(f"✅ Erweiterte Analyse gespeichert: {OUTPUT_JSON}")
print(f"{'='*80}")

# Statistik
new_count = sum(1 for k in all_results.keys() if 21 <= int(k) <= 40)
compatible = sum(1 for k, r in all_results.items() if 21 <= int(k) <= 40 and r['backtest_compatible'])
avg_combos = sum(r['calculated_combinations'] for k, r in all_results.items() if 21 <= int(k) <= 40) / new_count if new_count > 0 else 0
