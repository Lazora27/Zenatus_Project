# -*- coding: utf-8 -*-
"""
QUICK BACKTEST - 150 CLEAN STRATEGIEN
======================================
...
# Lade 150 Clean-IDs
CLEAN_IDS_FILE = BASE_PATH / "01_Backtest_System" / "Scripts" / "PROBLEM_AND_CLEAN_IDS.json"
with open(CLEAN_IDS_FILE, 'r') as f:
    ids_data = json.load(f)
CLEAN_IDS = ids_data['clean_ids']
...
# STANDARD PARAMETERS (mehr Combos für Clean-Strategien)
PERIOD_VALUES = [10, 20, 50]
TP_SL_COMBOS = [(50, 30), (100, 50), (150, 75)]

# Log-Datei
timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
LOG_FILE = LOG_PATH / f"quicktest_clean_150_{timestamp}.log"
...
log("="*80)
log(f"QUICK BACKTEST - 150 CLEAN STRATEGIEN")
log("="*80)
log(f"Daterange: {DATE_START} - {DATE_END}")
log(f"Clean-IDs: {len(CLEAN_IDS)}")
...
def main():
    log("\n" + "="*80)
    log("START QUICK BACKTEST - 150 CLEAN STRATEGIEN")
    log("="*80)
    ...
    # Teste alle Clean-Indikatoren
    all_results = []
    success_count = 0
    error_count = 0
    no_results_count = 0
    
    for i, ind_id in enumerate(CLEAN_IDS, 1):
        log(f"\n{'='*80}")
        log(f"FORTSCHRITT: {i}/{len(CLEAN_IDS)} ({i/len(CLEAN_IDS)*100:.1f}%)")
        log(f"{'='*80}")
    ...
    log("="*80)
    log(f"Total getestet: {len(CLEAN_IDS)}")
    log(f"SUCCESS: {success_count}")
    log(f"ERROR: {error_count}")
    log(f"NO RESULTS: {no_results_count}")
    log(f"Erfolgsquote: {success_count/len(CLEAN_IDS)*100:.1f}%")
    
    # Speichere Ergebnisse
    results_file = OUTPUT_PATH / "Scripts" / f"quicktest_clean_150_results_{timestamp}.json"
