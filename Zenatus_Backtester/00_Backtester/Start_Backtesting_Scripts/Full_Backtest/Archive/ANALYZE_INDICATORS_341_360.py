"""
DEEP-ANALYSE: Indikatoren 341-360
...
def analyze_indicator_deep(ind_file: Path) -> dict:
    ...
    if not result['parameters']:
        ...
        if gen_signals_match:
            ...
            if 'period' in sig:
                result['parameters']['period'] = {'min': 5, 'max': 200, 'default': 20}
    
    num_params = len(result['parameters'])
    
    if num_params == 0:
        result['optimal_inputs']['period'] = {
            'start': 5, 'end': 200, 'step': 13,
        ...
        result['optimal_inputs'][param_name] = {
            'start': start, 'end': end, 'step': step, 'values': values, 'count': len(values)
        }
        result['optimal_inputs']['tp_pips'] = {'values': [30, 50, 75, 110], 'count': 4}
        result['optimal_inputs']['sl_pips'] = {'values': [20, 30, 45, 70], 'count': 4}
        result['calculated_combinations'] = len(values) * 16
...
print("="*80)
print("DEEP-ANALYSE: INDIKATOREN 341-360")
print("="*80)
...
for i in range(341, 361):
    ...
    print(f"\n#{i:03d}: {ind_file.stem}")
    print(f"Klasse: {result['class_name']}, Param: {len(result['parameters'])}, Kombi: {result['calculated_combinations']}")
...
new_count = sum(1 for k in all_results.keys() if 341 <= int(k) <= 360)
avg = sum(r['calculated_combinations'] for k, r in all_results.items() if 341 <= int(k) <= 360) / new_count if new_count > 0 else 0
print(f"📊 (341-360): {new_count}/20, Ø {avg:.0f} Kombinationen")
print(f"📊 GESAMT (1-360): {len(all_results)} Indikatoren")
