"""
DEEP-ANALYSE: Indikatoren 181-200
...
# MAIN
print("="*80)
print("DEEP-ANALYSE: INDIKATOREN 181-200")
print("="*80)
...
# Statistik
new_count = sum(1 for k in all_results.keys() if 181 <= int(k) <= 200)
compatible = sum(1 for k, r in all_results.items() if 181 <= int(k) <= 200 and r['backtest_compatible'])
avg_combos = sum(r['calculated_combinations'] for k, r in all_results.items() if 181 <= int(k) <= 200) / new_count if new_count > 0 else 0
