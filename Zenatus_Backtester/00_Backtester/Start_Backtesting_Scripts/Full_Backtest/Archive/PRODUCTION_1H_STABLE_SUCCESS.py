...
DATE_END = '2025-09-20'
# STABLE_SUCCESS: Skip bereits getestete CSVs + Problem-Indikatoren
# Lade SKIP-Liste aus JSON (377 IDs total)
SKIP_LIST_JSON = BASE_PATH / "01_Backtest_System" / "Scripts" / "SKIP_LIST_CORRECT.json"
SKIP_INDICATORS = json.load(open(SKIP_LIST_JSON))['skip_indicators']
...
LEVERAGE = 10
POSITION_SIZE = 100
INDICATOR_TIMEOUT = 900  # 15 Minuten für stabile Indikatoren
...
print("="*80)
print(f"PRODUCTION STABLE_SUCCESS - {TIMEFRAME.upper()}")
print("="*80)
print(f"Skip Indicators: {len(SKIP_INDICATORS)} (117 getestet + 106 Problem + 154 andere)")
print(f"Zu testen: ~223 STABLE_SUCCESS Indikatoren")
print("="*80)
...
LOG_FILE = LOG_PATH / f"stable_success_{TIMEFRAME}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
