"""
DEEP-ANALYSE: Indikatoren 101-120
...
# MAIN
print("="*80)
print("DEEP-ANALYSE: INDIKATOREN 101-120")
print("="*80)
...
# Statistik
new_count = sum(1 for k in all_results.keys() if 101 <= int(k) <= 120)
compatible = sum(1 for k, r in all_results.items() if 101 <= int(k) <= 120 and r['backtest_compatible'])
avg_combos = sum(r['calculated_combinations'] for k, r in all_results.items() if 101 <= int(k) <= 120) / new_count if new_count > 0 else 0
