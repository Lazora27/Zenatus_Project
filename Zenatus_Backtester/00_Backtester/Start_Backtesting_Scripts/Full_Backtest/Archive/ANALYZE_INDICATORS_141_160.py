"""
DEEP-ANALYSE: Indikatoren 141-160
...
# MAIN
print("="*80)
print("DEEP-ANALYSE: INDIKATOREN 141-160")
print("="*80)
...
# Statistik
new_count = sum(1 for k in all_results.keys() if 141 <= int(k) <= 160)
compatible = sum(1 for k, r in all_results.items() if 141 <= int(k) <= 160 and r['backtest_compatible'])
avg_combos = sum(r['calculated_combinations'] for k, r in all_results.items() if 141 <= int(k) <= 160) / new_count if new_count > 0 else 0
