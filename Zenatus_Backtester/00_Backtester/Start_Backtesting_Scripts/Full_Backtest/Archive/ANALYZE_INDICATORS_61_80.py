"""
DEEP-ANALYSE: Indikatoren 61-80
...
# MAIN
print("="*80)
print("DEEP-ANALYSE: INDIKATOREN 61-80")
print("="*80)
...
# Statistik
new_count = sum(1 for k in all_results.keys() if 61 <= int(k) <= 80)
compatible = sum(1 for k, r in all_results.items() if 61 <= int(k) <= 80 and r['backtest_compatible'])
avg_combos = sum(r['calculated_combinations'] for k, r in all_results.items() if 61 <= int(k) <= 80) / new_count if new_count > 0 else 0
