"""
DEEP-ANALYSE: Indikatoren 281-300
...
print("="*80)
print("DEEP-ANALYSE: INDIKATOREN 281-300")
print("="*80)
...
new_count = sum(1 for k in all_results.keys() if 281 <= int(k) <= 300)
compatible = sum(1 for k, r in all_results.items() if 281 <= int(k) <= 300 and r['backtest_compatible'])
avg_combos = sum(r['calculated_combinations'] for k, r in all_results.items() if 281 <= int(k) <= 300) / new_count if new_count > 0 else 0

print(f"\n📊 STATISTIK (281-300): {new_count}/20 analysiert, {compatible} kompatibel, Ø {avg_combos:.0f} Kombinationen")
print(f"📊 GESAMT (1-300): {len(all_results)} Indikatoren")
