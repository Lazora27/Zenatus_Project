...
QUICKTEST_JSON = BASE_PATH / "01_Backtest_System" / "Scripts" / "QUICKTEST_PARAMS.json"
...
INDICATOR_TIMEOUT = 1200  # 20 Minuten pro Indikator (10min)
...
print("="*80)
print(f"EXTREME TIMEOUT TEST - 10 MINUTE TIMEOUT - {TIMEFRAME.upper()}")
print("="*80)
print(f"Date: {DATE_START} to {DATE_END}")
print(f"Symbols: {len(SYMBOLS)}")
print(f"Mode: QUICKTEST (2 combos per indicator)")
print(f"Workers: {MAX_WORKERS}")
print(f"Timeout: {INDICATOR_TIMEOUT}s (10 minutes)")
print(f"Testing: EXTREME TIMEOUT + FEW_SIGNALS indicators")
print("="*80)
...
LOG_FILE = LOG_PATH / f"extreme_timeout_10min_{TIMEFRAME}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def test_indicator_quick(ind_num):
    ...
    try:
        ...
        try:
            ...
            thread.daemon = True
            thread.start()
            thread.join(timeout=600)  # 10min: 30s -> 600s
        ...
        for tp_pips, sl_pips in tp_sl_combos:
            try:
                ...
                thread.daemon = True
                thread.start()
                thread.join(timeout=600)  # 10min: 60s -> 600s
...
log_message("Starting EXTREME TIMEOUT Test (10min)...", "INFO")
start_time = time.time()

# Load EXTREME_TIMEOUT_LIST
import json
with open(Path(__file__).parent / 'EXTREME_TIMEOUT_LIST.json', 'r') as f:
    extreme_list = json.load(f)['extreme_timeout_10min']

# Get only extreme timeout indicator numbers from QUICKTEST_PARAMS
indicator_nums = sorted([int(k) for k in QUICKTEST_PARAMS.keys() if int(k) in extreme_list])

log_message(f"Testing {len(indicator_nums)} EXTREME TIMEOUT indicators with 10min timeout", "INFO")
...
# Save results
results_file = OUTPUT_PATH / "EXTREME_TIMEOUT_10MIN_RESULTS.json"
