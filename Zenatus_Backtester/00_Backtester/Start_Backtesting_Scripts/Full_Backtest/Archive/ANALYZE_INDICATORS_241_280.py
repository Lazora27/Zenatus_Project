"""
DEEP-ANALYSE: Indikatoren 241-280 (kombiniert)
...
# MAIN
print("="*80)
print("DEEP-ANALYSE: INDIKATOREN 241-280 (KOMBINIERT)")
...
for i in range(241, 281):
    ...
    print(f"\nENTRY-PARAMETER (Start/Step/End):")
...
print(f"\n{'='*80}")
print(f"✅ Analyse gespeichert: {JSON_FILE}")
print(f"{'='*80}")

# Statistik
new_count = sum(1 for k in all_results.keys() if 241 <= int(k) <= 280)
compatible = sum(1 for k, r in all_results.items() if 241 <= int(k) <= 280 and r['backtest_compatible'])
avg_combos = sum(r['calculated_combinations'] for k, r in all_results.items() if 241 <= int(k) <= 280) / new_count if new_count > 0 else 0

print(f"\n📊 STATISTIK (241-280):")
print(f"  Analysiert: {new_count}/40")
print(f"  Backtest-Kompatibel: {compatible}/{new_count}")
print(f"  Durchschnitt Kombinationen: {avg_combos:.0f}")

print(f"\n📊 GESAMT-STATISTIK (1-280):")
