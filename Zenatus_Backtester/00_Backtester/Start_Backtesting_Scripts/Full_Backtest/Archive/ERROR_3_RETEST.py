...
QUICKTEST_JSON = BASE_PATH / "01_Backtest_System" / "Scripts" / "ERROR_3_RETEST.json"
...
print("="*80)
print(f"ERROR 3 RETEST - 2.5X TIMEOUT - {TIMEFRAME.upper()}")
print("="*80)
...
print(f"Testing: 3 fixed ERROR indicators (#272, #274, #347)")
print("="*80)
...
LOG_FILE = LOG_PATH / f"error_3_retest_{TIMEFRAME}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
log_message("Starting ERROR 3 RETEST (2.5x timeout)...", "INFO")
start_time = time.time()

# Get only 3 ERROR indicator numbers from QUICKTEST_PARAMS
indicator_nums = sorted([int(k) for k in QUICKTEST_PARAMS.keys()])

log_message(f"Testing {len(indicator_nums)} fixed ERROR indicators with 2.5x timeout", "INFO")
...
# Save results
results_file = OUTPUT_PATH / "ERROR_3_RETEST_RESULTS.json"
