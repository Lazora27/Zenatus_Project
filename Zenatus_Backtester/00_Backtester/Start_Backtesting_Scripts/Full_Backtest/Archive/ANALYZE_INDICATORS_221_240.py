"""
DEEP-ANALYSE: Indikatoren 221-240
...
def analyze_indicator_deep(ind_file: Path) -> dict:
    ...
    # 6. Berechne optimale Inputs mit PRÄZISEN Start/Step/End Werten
    num_params = len(result['parameters'])
    
    if num_params == 0:
        # Default: period mit präzisen Werten
        ...
        result['calculated_combinations'] = 16 * 4 * 4  # 256
        ...
        result['calculated_combinations'] = 8 * 8 * 3 * 3  # 576
        ...
        result['calculated_combinations'] = 6 * 6 * 6 * 2 * 2  # 864
        ...
        result['calculated_combinations'] = (5 ** 4) * 2 * 2  # 2500
...
# MAIN
print("="*80)
print("DEEP-ANALYSE: INDIKATOREN 221-240")
print("FOKUS: Start/Step/End Werte für Entry-Parameter")
print("="*80)
...
for i in range(221, 241):
    ...
    print(f"ENTRY-PARAMETER (Start/Step/End):")
    for param_name, param_config in result['optimal_inputs'].items():
        if param_name not in ['tp_pips', 'sl_pips']:
            print(f"  - {param_name}:")
            print(f"      Start: {param_config.get('start', 'N/A')}")
            print(f"      End: {param_config.get('end', 'N/A')}")
            print(f"      Step: {param_config.get('step', 'N/A')}")
            print(f"      Count: {param_config['count']} Werte")
    
    print(f"\nTP/SL (weniger wichtig):")
...
# Statistik
new_count = sum(1 for k in all_results.keys() if 221 <= int(k) <= 240)
compatible = sum(1 for k, r in all_results.items() if 221 <= int(k) <= 240 and r['backtest_compatible'])
avg_combos = sum(r['calculated_combinations'] for k, r in all_results.items() if 221 <= int(k) <= 240) / new_count if new_count > 0 else 0
