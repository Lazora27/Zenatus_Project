"""058 - Gator Oscillator (Bill Williams)"""
import numpy as np
import pandas as pd
...
class Indicator_GatorOscillator:
    """Gator Oscillator - Alligator Histogram"""
    PARAMETERS = {
    ...
    def __init__(self):
        self.name, self.category, self.version = "GatorOscillator", "Trend", __version__
    def calculate(self, data: pd.DataFrame, params: Dict) -> pd.DataFrame:
        """Gator = abs(Jaw - Teeth), abs(Teeth - Lips)"""
        ...
        return pd.DataFrame({'upper': upper, 'lower': lower, 'jaw': jaw, 'teeth': teeth, 'lips': lips}, index=data.index)
    def generate_signals_fixed(self, data: pd.DataFrame, params: Dict) -> Dict[str, pd.Series]:
        result = self.calculate(data, params)
        entries = (result['upper'] > result['upper'].shift(1)) & (result['lower'] > result['lower'].shift(1))
        ...
        return {'entries': entries, 'exits': exits, 'tp_levels': pd.Series(np.nan, index=data.index), 
                'sl_levels': pd.Series(np.nan, index=data.index), 'signal_strength': (result['upper'] + result['lower']).clip(0, 0.01) / 0.01}
    def generate_signals_dynamic(self, data: pd.DataFrame, params: Dict) -> Dict[str, pd.Series]:
        result = self.calculate(data, params)
        entries = (result['upper'] > result['upper'].shift(1)) & (result['lower'] > result['lower'].shift(1))
        exits = (result['upper'] < result['upper'].shift(1)) & (result['lower'] < result['lower'].shift(1))
        return {'entries': entries, 'exits': exits, 'exit_reason': pd.Series('gator_contraction', index=data.index), 'signal_strength': (result['upper'] + result['lower']).clip(0, 0.01) / 0.01}
    def get_ml_features(self, data: pd.DataFrame, params: Dict) -> pd.DataFrame:
        result = self.calculate(data, params)
        return pd.DataFrame({'gator_upper': result['upper'], 'gator_lower': result['lower'], 'gator_expanding': ((result['upper'] > result['upper'].shift(1)) & (result['lower'] > result['lower'].shift(1))).astype(int)}, index=data.index)
