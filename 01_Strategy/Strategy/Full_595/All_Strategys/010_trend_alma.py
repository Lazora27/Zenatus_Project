"""
010 - Arnaud Legoux Moving Average (ALMA)
Gaussian-gewichteter MA mit Offset
"""
...
class Indicator_ALMA:
    """
    Arnaud Legoux Moving Average (ALMA) - Trend
    
    Beschreibung:
        Gaussian-gewichteter MA mit verschiebbarem Offset.
        Kombiniert Smoothness mit geringem Lag.
    
    Formula:
        Gaussian weights mit Offset und Sigma
        ALMA = Sum(Price * Weights) / Sum(Weights)
    
    Parameters:
        - period: Lookback-Periode (2-200)
        - offset: Gaussian Offset (0.0-1.0, default 0.85)
        - sigma: Gaussian Sigma (1-10, default 6)
    ...
    PARAMETERS = {
        ...
        'offset': {
            ...
            'min': 0.0,
            'max': 1.0,
            'values': [0.5, 0.6, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95, 1.0],
            'step': 0.05,
            'optimize': True,
            'ml_feature': True
        },
    ...
    def calculate(self, data: pd.DataFrame, params: Dict) -> pd.Series:
        """Berechnet Arnaud Legoux Moving Average."""
        self.validate_params(params)
        
        period = params.get('period', self.PARAMETERS['period']['default'])
        offset = params.get('offset', self.PARAMETERS['offset']['default'])
        ...
        alma = data['close'].rolling(window=period).apply(alma_calc, raw=True)
        
        return alma
    
    def generate_signals_fixed(self, data: pd.DataFrame, params: Dict) -> Dict[str, pd.Series]:
        """A.a) Fixed TP/SL Strategie."""
        alma = self.calculate(data, params)
        entries = (data['close'] > alma) & (data['close'].shift(1) <= alma.shift(1))
        ...
        signal_strength = abs(data['close'] - alma) / alma
    ...
    def generate_signals_dynamic(self, data: pd.DataFrame, params: Dict) -> Dict[str, pd.Series]:
        ...
        alma = self.calculate(data, params)
        entries = (data['close'] > alma) & (data['close'].shift(1) <= alma.shift(1))
        exits = (data['close'] < alma) & (data['close'].shift(1) >= alma.shift(1))
        
        exit_reason = pd.Series('', index=data.index)
        exit_reason[exits] = 'alma_reverse_cross'
    ...
    def get_ml_features(self, data: pd.DataFrame, params: Dict) -> pd.DataFrame:
        ...
        features['alma_value'] = alma
        rolling_min = alma.rolling(100, min_periods=1).min()
        rolling_max = alma.rolling(100, min_periods=1).max()
        features['alma_normalized'] = (alma - rolling_min) / (rolling_max - rolling_min + 1e-10)
        features['alma_slope'] = alma.diff()
        features['alma_acceleration'] = alma.diff().diff()
        features['distance_from_price'] = data['close'] - alma
        features['distance_pct'] = (data['close'] - alma) / alma * 100
        features['above_alma'] = (data['close'] > alma).astype(int)
        ...
        for param_name, param_info in self.PARAMETERS.items():
            if param_info.get('optimize', False):
                grid[param_name] = param_info.get('values', list(range(
                    param_info['min'], param_info['max'] + 1, param_info.get('step', 1)
                )) if isinstance(param_info['min'], int) else param_info['values'])
        return grid
