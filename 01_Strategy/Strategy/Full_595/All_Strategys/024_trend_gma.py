"""024 - Geometric Moving Average (GMA)"""
import numpy as np
import pandas as pd
...
class Indicator_GMA:
    """Geometric Moving Average - Trend"""
    PARAMETERS = {
    ...
    def __init__(self):
        self.name, self.category, self.version = "GMA", "Trend", "1.0.0"
    
    def calculate(self, data: pd.DataFrame, params: Dict) -> pd.Series:
        period = params.get('period', 20)
        # GMA = (P1 * P2 * ... * Pn)^(1/n)
        log_prices = np.log(data['close'].replace(0, np.nan))
        gma = np.exp(log_prices.rolling(window=period).mean())
        return gma
    
    def generate_signals_fixed(self, data: pd.DataFrame, params: Dict) -> Dict:
        gma = self.calculate(data, params)
        entries = (data['close'] > gma) & (data['close'].shift(1) <= gma.shift(1))
        ...
        return {'entries': entries, 'exits': exits, 'tp_levels': tp_levels, 'sl_levels': sl_levels, 'signal_strength': abs(data['close'] - gma) / gma}
    
    def generate_signals_dynamic(self, data: pd.DataFrame, params: Dict) -> Dict:
        gma = self.calculate(data, params)
        entries = (data['close'] > gma) & (data['close'].shift(1) <= gma.shift(1))
        exits = (data['close'] < gma) & (data['close'].shift(1) >= gma.shift(1))
        return {'entries': entries, 'exits': exits, 'exit_reason': pd.Series('gma_cross', index=data.index), 'signal_strength': abs(data['close'] - gma) / gma}
    
    def get_ml_features(self, data: pd.DataFrame, params: Dict) -> pd.DataFrame:
        gma = self.calculate(data, params)
        features = pd.DataFrame(index=data.index)
        features['gma_value'], features['gma_slope'] = gma, gma.diff()
        features['distance_from_price'] = data['close'] - gma
        return features
