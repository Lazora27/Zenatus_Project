"""
016 - Sine Weighted Moving Average (SINWMA)
...
class Indicator_SINWMA:
    """Sine Weighted Moving Average (SINWMA) - Trend"""
    ...
    def calculate(self, data: pd.DataFrame, params: Dict) -> pd.Series:
        """Berechnet Sine Weighted MA."""
        self.validate_params(params)
        period = params.get('period', 20)
        
        # Sinus-Gewichte
        weights = np.array([np.sin((i + 1) * np.pi / (period + 1)) for i in range(period)])
        ...
        sinwma = data['close'].rolling(window=period).apply(sinwma_calc, raw=True)
        return sinwma
    
    def generate_signals_fixed(self, data: pd.DataFrame, params: Dict) -> Dict[str, pd.Series]:
        sinwma = self.calculate(data, params)
        entries = (data['close'] > sinwma) & (data['close'].shift(1) <= sinwma.shift(1))
        ...
        return {'entries': entries, 'exits': exits,
                ...
                'signal_strength': abs(data['close'] - sinwma) / sinwma}
    
    def generate_signals_dynamic(self, data: pd.DataFrame, params: Dict) -> Dict[str, pd.Series]:
        sinwma = self.calculate(data, params)
        entries = (data['close'] > sinwma) & (data['close'].shift(1) <= sinwma.shift(1))
        exits = (data['close'] < sinwma) & (data['close'].shift(1) >= sinwma.shift(1))
        exit_reason = pd.Series('', index=data.index)
        exit_reason[exits] = 'sinwma_reverse_cross'
    ...
    def get_ml_features(self, data: pd.DataFrame, params: Dict) -> pd.DataFrame:
        ...
        features['sinwma_value'] = sinwma
        features['sinwma_slope'] = sinwma.diff()
        features['distance_from_price'] = data['close'] - sinwma
        return features
