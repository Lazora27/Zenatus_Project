"""
019 - Linear Regression Moving Average
Linear Regression Trendlinie
"""
...
class Indicator_LINREG:
    """Linear Regression Moving Average - Trend"""
    ...
    def calculate(self, data: pd.DataFrame, params: Dict) -> pd.Series:
        """Berechnet Linear Regression MA."""
        ...
        def linreg_calc(x):
            ...
            # Current value (last point on regression line)
            return slope * (len(y) - 1) + intercept
        
        linreg = data['close'].rolling(window=period).apply(linreg_calc, raw=True)
        return linreg
    
    def generate_signals_fixed(self, data: pd.DataFrame, params: Dict) -> Dict[str, pd.Series]:
        linreg = self.calculate(data, params)
        entries = (data['close'] > linreg) & (data['close'].shift(1) <= linreg.shift(1))
        ...
        return {'entries': entries, 'exits': exits,
                ...
                'signal_strength': abs(data['close'] - linreg) / linreg}
    
    def generate_signals_dynamic(self, data: pd.DataFrame, params: Dict) -> Dict[str, pd.Series]:
        linreg = self.calculate(data, params)
        entries = (data['close'] > linreg) & (data['close'].shift(1) <= linreg.shift(1))
        exits = (data['close'] < linreg) & (data['close'].shift(1) >= linreg.shift(1))
        exit_reason = pd.Series('', index=data.index)
        exit_reason[exits] = 'linreg_reverse_cross'
    ...
    def get_ml_features(self, data: pd.DataFrame, params: Dict) -> pd.DataFrame:
        ...
        features['linreg_value'] = linreg
        features['linreg_slope'] = linreg.diff()
        features['distance_from_price'] = data['close'] - linreg
        return features
