"""
009 - Zero Lag EMA (ZLEMA)
Eliminiert Lag durch Lag-Kompensation
"""
...
class Indicator_ZLEMA:
    """
    Zero Lag Exponential Moving Average (ZLEMA) - Trend
    
    Beschreibung:
        EMA mit Lag-Kompensation. Nutzt de-lagged Daten.
        ZLEMA = EMA(Close + (Close - Close[lag]))
    
    Formula:
        lag = (period - 1) / 2
        data = Close + (Close - Close[lag])
        ZLEMA = EMA(data, period)
    ...
    def calculate(self, data: pd.DataFrame, params: Dict) -> pd.Series:
        ...
        # De-lagged Daten
        delagged = data['close'] + (data['close'] - data['close'].shift(lag))
        
        # EMA auf de-lagged Daten
    ...
    def generate_signals_fixed(self, data: pd.DataFrame, params: Dict) -> Dict[str, pd.Series]:
        ...
        zlema = self.calculate(data, params)
        entries = (data['close'] > zlema) & (data['close'].shift(1) <= zlema.shift(1))
        ...
        signal_strength = abs(data['close'] - zlema) / zlema
    ...
    def generate_signals_dynamic(self, data: pd.DataFrame, params: Dict) -> Dict[str, pd.Series]:
        ...
        zlema = self.calculate(data, params)
        entries = (data['close'] > zlema) & (data['close'].shift(1) <= zlema.shift(1))
        exits = (data['close'] < zlema) & (data['close'].shift(1) >= zlema.shift(1))
        
        exit_reason = pd.Series('', index=data.index)
        exit_reason[exits] = 'zlema_reverse_cross'
    ...
    def get_ml_features(self, data: pd.DataFrame, params: Dict) -> pd.DataFrame:
        ...
        features['zlema_value'] = zlema
        rolling_min = zlema.rolling(100, min_periods=1).min()
        rolling_max = zlema.rolling(100, min_periods=1).max()
        features['zlema_normalized'] = (zlema - rolling_min) / (rolling_max - rolling_min + 1e-10)
        features['zlema_slope'] = zlema.diff()
        features['zlema_acceleration'] = zlema.diff().diff()
        features['distance_from_price'] = data['close'] - zlema
        features['distance_pct'] = (data['close'] - zlema) / zlema * 100
        features['above_zlema'] = (data['close'] > zlema).astype(int)
