"""
003 - Weighted Moving Average (WMA)
...
class Indicator_WMA:
    """
    Weighted Moving Average (WMA) - Trend
    
    Beschreibung:
        Linear gewichteter Durchschnitt. Neueste Preise haben höchstes Gewicht.
        Gewichte: N, N-1, N-2, ..., 1
    ...
    PARAMETERS = {
        'period': {
            ...
            'optimize': True,
            'ml_feature': True,
            'description': 'WMA lookback period'
        },
        'tp_pips': {
            'default': 50,
            'min': 20,
    ...
    def calculate(self, data: pd.DataFrame, params: Dict) -> pd.Series:
        """
        Berechnet Weighted Moving Average.
        ...
        wma = data['close'].rolling(window=period).apply(wma_calc, raw=True)
        return wma
    
    def generate_signals_fixed(self, data: pd.DataFrame, params: Dict) -> Dict[str, pd.Series]:
        """A.a) Fixed TP/SL Strategie."""
        wma = self.calculate(data, params)
        entries = (data['close'] > wma) & (data['close'].shift(1) <= wma.shift(1))
        ...
        signal_strength = abs(data['close'] - wma) / wma
    ...
    def generate_signals_dynamic(self, data: pd.DataFrame, params: Dict) -> Dict[str, pd.Series]:
        ...
        wma = self.calculate(data, params)
        entries = (data['close'] > wma) & (data['close'].shift(1) <= wma.shift(1))
        exits = (data['close'] < wma) & (data['close'].shift(1) >= wma.shift(1))
        
        exit_reason = pd.Series('', index=data.index)
        exit_reason[exits] = 'wma_reverse_cross'
    ...
    def get_ml_features(self, data: pd.DataFrame, params: Dict) -> pd.DataFrame:
        ...
        features['wma_value'] = wma
        rolling_min = wma.rolling(100, min_periods=1).min()
        rolling_max = wma.rolling(100, min_periods=1).max()
        features['wma_normalized'] = (wma - rolling_min) / (rolling_max - rolling_min + 1e-10)
        features['wma_slope'] = wma.diff()
        features['wma_acceleration'] = wma.diff().diff()
        features['distance_from_price'] = data['close'] - wma
        features['distance_pct'] = (data['close'] - wma) / wma * 100
        features['above_wma'] = (data['close'] > wma).astype(int)
