...
def render_metrics_html(date_str: str, folders: int, counts: dict, ext_list: list, history: dict, base_dir: str):
    ...
    html.append("".join([f"<span class='badge' style='background:{color_for_ext(ext)}'>{ext if ext else '(no ext)'}: {counts.get(ext,0)}</span>" for ext in sorted(ext_list, key=lambda e: counts.get(e,0), reverse=True)]))
    html.append("</div>")
    html.append("</div>")
...
def main():
    ...
    prev_week_date = closest_before(cur_dt - timedelta(days=7))
    prev_month_date = closest_before(cur_dt - timedelta(days=30))
    ...
    def render_timeline_html(hist_obj):
        ...
        html.append(".legend{display:flex;flex-wrap:wrap;gap:8px;margin:10px 0}")
        html.append(".legend .item{display:flex;align-items:center;gap:6px;padding:4px 8px;border:1px solid #eee;border-radius:8px;background:#fff;font-size:12px}")
        html.append(".legend .swatch{width:12px;height:12px;border-radius:3px}")
        ...
        html.append("<div class='chart'><div class='scroll'><canvas id='extChart' height='300'></canvas></div><div class='legend' id='extLegend'></div></div>")
...
function renderLegend(containerId, extData){
  ...
  keys.forEach((k,ki)=>{
    ...
    const sw=document.createElement('div'); sw.className='swatch'; sw.style.background=colors[ki%colors.length];
    const label=document.createElement('div'); label.textContent=k || '(no ext)';
    item.appendChild(sw); item.appendChild(label); cont.appendChild(item);
  });
}
...
renderLegend('extLegend', timelineData.ext);
""")
