...
ANALYSIS_JSON = BASE_PATH / "01_Backtest_System" / "Scripts" / "INDICATORS_COMPLETE_ANALYSIS.json"  # Deep Analysis Results
...
DATE_END = '2025-09-20'
# UNTESTED_39 MODE: Nur 39 ungetestete SUCCESS Indikatoren
import json
with open(Path(__file__).parent / 'UNTESTED_SUCCESS.json', 'r') as f:
    untested_success = json.load(f)['untested_success']
all_possible = list(range(1, 601))
SKIP_INDICATORS = [x for x in all_possible if x not in untested_success]
...
LEVERAGE = 10
POSITION_SIZE = 100
INDICATOR_TIMEOUT = 1200  # 20 Minuten pro Indikator
MAX_WORKERS = 3  # Reduziert für stabilere Performance
...
print("="*80)
print(f"PRODUCTION UNTESTED_39 - {TIMEFRAME.upper()}")
print("="*80)
print(f"Testing: {len(untested_success)} ungetestete SUCCESS Indikatoren")
print(f"IDs: {sorted(untested_success)[:10]}...")
print("="*80)
...
LOG_FILE = LOG_PATH / f"production_untested_39_{TIMEFRAME}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
