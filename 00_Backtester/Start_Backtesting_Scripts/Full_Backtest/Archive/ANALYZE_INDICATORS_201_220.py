"""
DEEP-ANALYSE: Indikatoren 201-220
...
# MAIN
print("="*80)
print("DEEP-ANALYSE: INDIKATOREN 201-220")
print("="*80)
...
# Statistik
new_count = sum(1 for k in all_results.keys() if 201 <= int(k) <= 220)
compatible = sum(1 for k, r in all_results.items() if 201 <= int(k) <= 220 and r['backtest_compatible'])
avg_combos = sum(r['calculated_combinations'] for k, r in all_results.items() if 201 <= int(k) <= 220) / new_count if new_count > 0 else 0
