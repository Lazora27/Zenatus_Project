...
INDICATOR_TIMEOUT = 300  # 5 Minuten pro Indikator (2.5x)
...
print("="*80)
print(f"TIMEOUT TEST - 2.5X TIMEOUT - {TIMEFRAME.upper()}")
print("="*80)
print(f"Date: {DATE_START} to {DATE_END}")
print(f"Symbols: {len(SYMBOLS)}")
print(f"Mode: QUICKTEST (2 combos per indicator)")
print(f"Workers: {MAX_WORKERS}")
print(f"Timeout: {INDICATOR_TIMEOUT}s (2.5x increased)")
print(f"Testing: TIMEOUT_SIGNALS indicators only")
print("="*80)
...
LOG_FILE = LOG_PATH / f"timeout_test_{TIMEFRAME}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def test_indicator_quick(ind_num):
    ...
    try:
        ...
        try:
            ...
            thread.daemon = True
            thread.start()
            thread.join(timeout=75)  # 2.5x: 30s -> 75s
        ...
        for tp_pips, sl_pips in tp_sl_combos:
            try:
                ...
                thread.daemon = True
                thread.start()
                thread.join(timeout=150)  # 2.5x: 60s -> 150s
...
log_message("Starting TIMEOUT Test...", "INFO")
start_time = time.time()

# Load previous results to get TIMEOUT_SIGNALS list
import json
results_file = OUTPUT_PATH / "QUICKTEST_RESULTS.json"
with open(results_file, 'r', encoding='utf-8') as f:
    previous_results = json.load(f)

timeout_signals = previous_results.get('TIMEOUT_SIGNALS', [])

# Get only TIMEOUT_SIGNALS indicator numbers from QUICKTEST_PARAMS
indicator_nums = sorted([int(k) for k in QUICKTEST_PARAMS.keys() if int(k) in timeout_signals])

log_message(f"Testing {len(indicator_nums)} TIMEOUT_SIGNALS indicators with 2.5x timeout", "INFO")
...
# Save results
results_file = OUTPUT_PATH / "TIMEOUT_TEST_RESULTS.json"
