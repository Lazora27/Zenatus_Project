...
HANDBOOK_JSON = BASE_PATH / "01_Strategy" / "Parameter_Optimization" / "PARAMETER_HANDBOOK_COMPLETE.json"
SUMMARY_CSV = BASE_PATH / "01_Strategy" / "Parameter_Optimization" / "PARAMETER_SUMMARY.csv"
...
INDICATOR_TIMEOUT = 120  # 2 Minuten pro Indikator
...
print("="*80)
print(f"QUICKTEST - PIPELINE VALIDATION - {TIMEFRAME.upper()}")
print("="*80)
...
LOG_FILE = LOG_PATH / f"quicktest_{TIMEFRAME}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
log_message("Loading Parameter Handbook", "INFO")
...
log_message(f"Loaded {len(HANDBOOK_INDEX)} indicators from handbook", "INFO")
SUMMARY_DF = pd.read_csv(SUMMARY_CSV)
SUMMARY_INDICATORS = [int(x) for x in SUMMARY_DF['Indicator_Num'].tolist() if not pd.isna(x)]
INDICATOR_LIST = [i for i in SUMMARY_INDICATORS if i in HANDBOOK_INDEX]
...
def test_indicator_quick(ind_num):
    ...
    if int(ind_num) not in HANDBOOK_INDEX:
        return None
    handbook = HANDBOOK_INDEX[int(ind_num)]
    ind_name = handbook['Indicator_Name']
    ind_file = INDICATORS_PATH / f"{ind_name}.py"
    ...
    try:
        ...
        # Extract first 2 combinations from Handbook
        entry_params = {}
        entry_cfg = handbook.get('Entry_Params', {})
        for param_name, param_config in entry_cfg.items():
            values = param_config.get('values', [])
            if isinstance(values, list) and len(values) >= 1:
                entry_params[param_name] = values[0]
        exit_cfg = handbook.get('Exit_Params', {})
        tp_values = exit_cfg.get('tp_pips', {}).get('values', [50, 100])[:2]
        sl_values = exit_cfg.get('sl_pips', {}).get('values', [30, 50])[:2]
...
# Get all indicator numbers from Summary (filtered by Handbook)
indicator_nums = sorted(INDICATOR_LIST)
