...
QUICKTEST_JSON = BASE_PATH / "01_Backtest_System" / "Scripts" / "QUICKTEST_PARAMS_FIXED.json"
...
print("="*80)
print(f"ERROR FIX TEST - FIXED PARAMETERS - {TIMEFRAME.upper()}")
print("="*80)
...
print(f"Testing: ERROR_SIGNALS indicators only")
print("="*80)
...
LOG_FILE = LOG_PATH / f"error_fix_test_{TIMEFRAME}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
log_message("Starting ERROR FIX Test...", "INFO")
start_time = time.time()

# Load previous results to get ERROR_SIGNALS list
import json
results_file = OUTPUT_PATH / "QUICKTEST_RESULTS.json"
with open(results_file, 'r', encoding='utf-8') as f:
    previous_results = json.load(f)

error_signals = previous_results.get('ERROR_SIGNALS', [])

# Get only ERROR_SIGNALS indicator numbers from QUICKTEST_PARAMS
indicator_nums = sorted([int(k) for k in QUICKTEST_PARAMS.keys() if int(k) in error_signals])

log_message(f"Testing {len(indicator_nums)} ERROR_SIGNALS indicators", "INFO")
...
# Save results
results_file = OUTPUT_PATH / "ERROR_FIX_RESULTS.json"
