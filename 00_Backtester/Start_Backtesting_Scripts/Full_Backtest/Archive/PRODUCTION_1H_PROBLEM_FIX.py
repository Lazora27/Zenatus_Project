...
INDICATORS_PATH = BASE_PATH / "00_Core" / "Indicators" / "Backup_04" / "Unique"  # 467 Unique Indicators
PARAM_CONFIGS_PATH = BASE_PATH / "00_Core" / "Indicators" / "Indicator_Configs"  # Individual Parameter JSONs
ANALYSIS_JSON = BASE_PATH / "01_Backtest_System" / "Scripts" / "INDICATORS_PROBLEM_2COMBOS.json"  # Problem-Indikatoren mit 2 Combos max
...
DATE_END = '2025-09-20'
# NUR Problem-Indikatoren testen (106 total)
# Lade aus JSON
SKIP_LIST_JSON = BASE_PATH / "01_Backtest_System" / "Scripts" / "SKIP_LIST_CORRECT.json"
skip_data = json.load(open(SKIP_LIST_JSON))
PROBLEM_IDS = skip_data['problem_indicators']
all_inds = set(range(1, 467))
SKIP_INDICATORS = sorted(all_inds - set(PROBLEM_IDS))
...
LEVERAGE = 10
POSITION_SIZE = 100
INDICATOR_TIMEOUT = 900  # 15 Minuten für Problem-Indikatoren (neuer Ansatz)
MAX_WORKERS = 6  # 6 Strategien parallel pro Thread/Kern
SLEEP_BETWEEN_INDICATORS = 3600  # 1 Stunde Sleep zwischen Indikatoren
...
print("="*80)
print(f"PROBLEM FIX BACKTEST - 15MIN TIMEOUT - {TIMEFRAME.upper()}")
print("="*80)
print(f"Problem-Indikatoren: {len(PROBLEM_IDS)}")
print(f"MAX 1-2 Kombinationen pro Indikator")
print(f"Timeout: 15 Minuten pro Indikator")
print(f"MAX_WORKERS=6 (parallel)")
print("="*80)
...
LOG_FILE = LOG_PATH / f"problem_fix_{TIMEFRAME}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
    ...
    for future in as_completed(futures):
        try:
            ...
            if result:
                completed_count += 1
                # 1 Stunde Sleep nach jedem erfolgreichen Indikator
                if completed_count < len(remaining_indicators):
                    sleep_hours = SLEEP_BETWEEN_INDICATORS // 3600
                    log_message(f"Sleep {sleep_hours} Stunde(n) vor nächstem Indikator...", "INFO")
                    print(f"\n⏸️  Sleep {sleep_hours}h vor nächstem Indikator...")
