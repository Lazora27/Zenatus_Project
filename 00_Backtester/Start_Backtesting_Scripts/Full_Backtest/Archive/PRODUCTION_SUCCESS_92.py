...
DATE_END = '2025-09-20'
# NUR SUCCESS Indikatoren (92) die noch nicht Full-Backtest haben
# Lade Liste aus SUCCESS_REMAINING.json
import json
with open(Path(__file__).parent / 'SUCCESS_REMAINING.json', 'r') as f:
    remaining_success = json.load(f)['remaining_success_indicators']

# Skip alle AUSSER remaining_success
all_possible = list(range(1, 601))
SKIP_INDICATORS = [x for x in all_possible if x not in remaining_success]
...
print("="*80)
print(f"PRODUCTION SUCCESS 92 - FULL BACKTEST - {TIMEFRAME.upper()}")
print("="*80)
print(f"Testing: {len(remaining_success)} SUCCESS indicators (not yet completed)")
...
LOG_FILE = LOG_PATH / f"production_success_92_{TIMEFRAME}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
