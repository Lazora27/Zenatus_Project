"""
DEEP-ANALYSE: Indikatoren 41-60
...
# MAIN
print("="*80)
print("DEEP-ANALYSE: INDIKATOREN 41-60")
print("="*80)
...
# Statistik
new_count = sum(1 for k in all_results.keys() if 41 <= int(k) <= 60)
compatible = sum(1 for k, r in all_results.items() if 41 <= int(k) <= 60 and r['backtest_compatible'])
avg_combos = sum(r['calculated_combinations'] for k, r in all_results.items() if 41 <= int(k) <= 60) / new_count if new_count > 0 else 0
