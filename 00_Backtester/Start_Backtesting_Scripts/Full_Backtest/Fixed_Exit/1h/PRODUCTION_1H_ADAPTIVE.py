...
PARAM_CONFIGS_PATH = BASE_PATH / "01_Strategy" / "Parameter_Optimization"
...
def calculate_metrics(pf, spread_pips):
    ...
    if len(equity_daily) > 1:
        ...
        worst_day_loss = (max(0.0, -daily_returns.min()) * 100) if len(daily_returns) > 0 else 0.0
    ...
    try:
        ...
        if best_combo and len(all_combo_results) > 0:
            ...
            if full_rows:
                ...
                log(f"Ind#{ind_num:03d} | {ind_name:30s} | {len(SYMBOLS)} symbols | {total_combos} combos | {elapsed:.1f}s | Best: SR={best_full['Sharpe_Ratio']:.2f}, PF={best_full['Profit_Factor']:.2f}, Ret={best_full['Total_Return']:.2f}%, DD={best_full['Max_Drawdown']:.2f}%")
        else:
            print(f"[{datetime.now().strftime('%H:%M:%S')}] Ind#{ind_num:03d} | {ind_name[:30]:30s} | NO RESULTS | {elapsed:.1f}s")
            log(f"Ind#{ind_num:03d} | {ind_name[:30]:30s} | NO RESULTS | {elapsed:.1f}s")
